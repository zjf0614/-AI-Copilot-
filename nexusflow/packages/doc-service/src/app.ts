...
export async function buildApp() {
  ...
  await app.register(swagger, { openapi: { info: { title: 'NexusFlow Doc Service', version: '1.0.0' } } });
  ...
  await app.register(docRoutes, { prefix: '/api/v1/workspaces' });
  app.get('/health', async () => ({ status: 'ok', service: 'doc-service', timestamp: new Date().toISOString() }));
  return app;
}
