...
export async function authenticate(request: FastifyRequest, _reply: FastifyReply) {
  ...
  try { request.user = jwt.verify(parts[1]!, config.JWT_PUBLIC_KEY, { algorithms: ['RS256'], issuer: config.JWT_ISSUER }) as AccessTokenPayload; }
  catch (err: any) { throw new AppError(ErrorCode.UNAUTHORIZED, err.name === 'TokenExpiredError' ? 'Token expired' : 'Invalid token', 401); }
}
